...
bool is_suspected_sql_injection(const std::string& sql) {
    // Regular expression for detecting "OR value=value;" pattern
    std::regex sql_injection_pattern(R"((\bOR\b\s+\d+=\d+;|\bOR\b\s+'.*'=+'.*';))", std::regex::icase);
    return std::regex_search(sql, sql_injection_pattern);
}
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    if (is_suspected_sql_injection(sql)) {
        std::cout << "Suspected SQL Injection detected. Query aborted: " << sql << std::endl;
        return false;
    }
